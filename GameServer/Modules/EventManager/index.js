...
EventManager.prototype.ReloadAllEvent = function () {
    ...
    this.GameServer.SQLAdapter.eventReloadAll(function (e, d) {
        if (!e) {
...
EventManager.prototype.EvtOnUserOutGame = function (User) {
    for (var i = 0; i < this.RunningEvent.length; i++) {
        this.RunningEvent[i].EvtOnUserOutGame(User);
    }
};

EventManager.prototype.EvtOnBossDie = function (ListUserHitBoss) {
    for (var i = 0; i < this.RunningEvent.length; i++) {
        this.RunningEvent[i].EvtOnBossDie(ListUserHitBoss);
    }
};