...
DeskCore.prototype.CalcWinResult = function () {
    ...
    for (var i = 0; i < this.ResultDices.length; i++) {
        redCount += +this.ResultDices[i];
    }
    if (redCount % 2 == 0) {//Chẵn
        rs.push(this.BetRatioConfig.Chan.ID);
    } else {
        rs.push(this.BetRatioConfig.Le.ID);
    }
    switch (redCount) {
        case 0:
            rs.push(this.BetRatioConfig.Red0.ID);
            break;
        case 1:
            rs.push(this.BetRatioConfig.Red1.ID);
            break;
        case 3:
            rs.push(this.BetRatioConfig.Red3.ID);
            break;
        case 4:
            rs.push(this.BetRatioConfig.Red4.ID);
            break;
    }
    return rs;
};
DeskCore.prototype.CalcWinForUser = function () {
    ...
    for (var i = 0; i < this.ListUserBet.length; i++) {//Check từng User
        ...
        if (Utility.CheckVariable(User, 'ListBet')) {
            ...
            this.CollectUserWinLose(User, totalBet, totalWin,history.BetList);
...
DeskCore.prototype.RecordUserBetHistory = function (UserID, HistoryStr) {
    try {
        this.GameServer.RedisAdapter.zAdd(this.GameServer.Config.RedisConfig.Define.XocDiaBetUserHistory + UserID, +new Date(), HistoryStr);
        this.GameServer.RedisAdapter.zCount(this.GameServer.Config.RedisConfig.Define.XocDiaBetUserHistory + UserID, function (e, d) {
            if (!e) {
                if (d > this.MaxBetHistory) {
                    var rem = d - this.MaxBetHistory;
                    this.GameServer.RedisAdapter.zRemByRank(this.GameServer.Config.RedisConfig.Define.XocDiaBetUserHistory + UserID, 0, rem);
                }
            }
        }.bind(this));
    } catch (e) {
...
DeskCore.prototype.UserGetUserHistory = function (User) {
    this.GameServer.RedisAdapter.zRange(this.GameServer.Config.RedisConfig.Define.XocDiaBetUserHistory + User.UserID, 0, -1, function (err, d) {
        if (!err) {
            try {
                var h = [];
...
DeskCore.prototype.CollectUserWinLose = function (User, TotalBet, TotalWin, BetList) {
    try {
        this.GameServer.RedisAdapter.zAdd(this.GameServer.Config.RedisConfig.Define.XocDiaTopUserWin, TotalWin - TotalBet, User.GameName);
        this.GameServer.EventManager.EvtOnUserWinLoseAll(User, TotalBet, TotalWin, BetList);
    } catch (e) {
...
DeskCore.prototype.BuildTopWinList = function () {
    this.GameServer.RedisAdapter.zRevRangeWithScores(this.GameServer.Config.RedisConfig.Define.XocDiaTopUserWin, 0, 50, function (err, d) {
        if (!err) {
            try {
                var gamename = 0;
                var score = 0;
