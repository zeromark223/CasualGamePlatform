...
DeskCore.prototype.UserEnterDesk = function (Socket) {
    try {
        ...
        if (User.MoneyState == this.GameServer.MoneyStateDefine.GettingCompleted) {
            ...
            User.InitDataRsID(); //Init data Record dùng cho event
...
DeskCore.prototype.OnUserSpin = function (User, GoldBet) {
    try {
        ...
        if (User.Wallet > GoldBet) {
            ...
            User.OnUserWinLoseGold(User, GoldBet, 0);
...
DeskCore.prototype.CalcResult = function (User, GoldBet, GoldBefore) {
    var cards = this.GetCardsFromPool(5);//Lấy 5 card
    this.CalcUserWin(User, cards, GoldBet, GoldBefore);
};

DeskCore.prototype.CalcUserWin = function (User, Cards, GoldBet, GoldBefore) {
    var ResultList = this.CardsToCardID(Cards);
    Cards = Cards.sort(function (a, b) {
        return a.CardNum - b.CardNum;
    });

    var BetRatioCfg = this.CheckRule(Cards);
    this.GameServer.Logger.Game().debug('DeskCore.prototype.CalcUserWin ', Cards, BetRatioCfg);
    var winGold = 0;
    var winId = -1;
    var isJackpot = 0;
    if (BetRatioCfg != 0) {
        if (BetRatioCfg.Ratio == 'j') {
            isJackpot = 1;
            winGold = 0;
        } else {
            winGold = +BetRatioCfg.Ratio * +GoldBet;
        }
        winId = +BetRatioCfg.ID;

    }

    User.RecordDataRsID(ResultList,GoldBet);

    if (isJackpot) {
        //handle get jackpot
        var jackpot = Math.floor(Math.random() * 10000000 + 1000000);
        this.GameServer.JackpotGetAndSet(User, GoldBet, function (jackpotValue) {
            if (jackpotValue > 0) { //Có jackpot
                this.HandleUserWin(User, jackpot, ResultList, GoldBefore, GoldBet, winId, isJackpot);
            } else {//đéo có jackpot, gọi tính toán lại
                this.CalcResult(User, GoldBet, GoldBefore);
            }
        }.bind(this));

    } else {
        this.HandleUserWin(User, winGold, ResultList, GoldBefore, GoldBet, winId, isJackpot);
    }
...
DeskCore.prototype.HandleUserWin = function (User, winGold, ResultList, GoldBefore, GoldBet, winId, isJackpot) {
    User.Wallet += +winGold;
    User.OnUserWinLoseGold(User, 0, winGold);
...
DeskCore.prototype.CheckSanh = function (Cards) {
    ...
    for (var i = 0; i < Cards.length - 1; i++) {
        if (Cards[i].CardNum == 0) {//Case xì
            if (Cards[i + 1].CardNum != 9 && Cards[i + 1].CardNum != 1) {// lá kế ko phải là J;
                return false;
            }
        } else {
...
DeskCore.prototype.CheckDoiJ = function (Cards) {
    ...
    for (var i = 0; i < t.length; i++) {
        if (t[i] > 1 ) {// count > 1 và || cuc5 nay2 bo roi&& (i > 9 || i == 0) cardnum > 10
            return true;
        }
    }
    return false;
...
DeskCore.prototype.UpdateUserLeftDesk = function (Socket) {
    ...
    this.GameServer.EventManager.EvtOnUserOutGameAll(Socket.UserModel);
