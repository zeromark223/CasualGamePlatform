...
module.exports = FourSpirit;
FourSpirit.prototype = new GameEvents();

FourSpirit.prototype.EvtOnUserEndMatch = function (User, BetList, WinList) {
    try {
...
FourSpirit.prototype.EvtOnUserEnter = function (User, DisConnectedUser) {
    if (!this.Run) return; // Event Kết thúc
...
FourSpirit.prototype.EvtInitUserDataEvent = function (User) {
    this.ResetUserData(User, 0);
};

FourSpirit.prototype.CheckRuleAntiCheat = function (BetList) {
    var isCheat = 0;
    if (this.FindBetID('tai', BetList) != 0 && this.FindBetID('xiu', BetList) != 0) {
        isCheat = 1;
    }
    if (this.FindBetID('chan', BetList) != 0 && this.FindBetID('le', BetList) != 0) {
        isCheat = 1;
    }
    return isCheat;
};

FourSpirit.prototype.FindBetID = function (ID, BetList) {
...
FourSpirit.prototype.CheckAndGetReward = function (User) {
    try {//Record
        var WinTrack = [];
        var i;
...
FourSpirit.prototype.ResetUserData = function (User, maxBet) {
    if (!User.FourSpiritData) {
        User.FourSpiritData = {};
    }
...
FourSpirit.prototype.AddRewardForUser = function (User, Reward, SpiritName, Config) {
    var isGet = 0;
...
FourSpirit.prototype.OnTurnOnTrigger = function () {
    try {
...
FourSpirit.prototype.EvtOnUserOutGame = function (User) {
    // if (!this.Run) return;
    // try {
    //     //#3
    //     this.GameServer.RedisAdapter.hset(this.DataKey, User.UserID + '', JSON.stringify(User.FourSpiritData));
...
FourSpirit.prototype.RecordEvent = function (User, Reward, RewardConfig, isGet) {
