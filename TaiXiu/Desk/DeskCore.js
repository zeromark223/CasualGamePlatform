...
DeskCore.prototype.EndRound = function () {
    try {
        ...
        if (Utility.CheckVariable(rs, 'Win', 'Lose', 'Tax')) {
            this.GameServer.RecordMatchInfo(this.MatchID, this.DeskID, this.ResultDices, this.ResultText, this.ResultTextEncrypted, rs.Win, rs.Lose, rs.Tax, this.GameServer.BetLimitConfig.Tax)
        }
    ...
    for (var i = 0; i < this.ListUserBet.length; i++) {//Check từng User
        ...
        if (Utility.CheckVariable(User, 'ListBet')) {
            for (var j = 0; j < User.ListBet.length; j++) {// Check từng Bet
                ...
                var BetWinByID = this.CheckBetIDWin(WinList, User.ListBet[j].ID);
                if (BetWinByID > 0) {
                    var winRatio = this.GetBetRatioByID(User.ListBet[j].ID) * BetWinByID;
            ...
            this.CollectUserWinLose(User, totalBet, totalWin);
...
DeskCore.prototype.CollectUserWinLose = function (User, TotalBet, TotalWin) {
    try{
        this.GameServer.RedisAdapter.zAdd(this.GameServer.Config.RedisConfig.Define.TaiXiuTopUserWin, TotalWin - TotalBet, User.GameName);
