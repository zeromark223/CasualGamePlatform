...
function loadScriptIncrbyLua() {
    var script = fs.readFileSync('./script/hincryby.lua');

    client.send_command('script', ['load', script], function (err, sha) {
        if (!err) {
            scriptList['IncrbyLua'] = sha;
        } else {
            console.log("loadScriptIncrbyLua error ", err);
        }
    }.bind(this));
}

function loadScriptHitGlobalBoss() {
    var script = fs.readFileSync('./script/hitGlobalBoss.lua');

    client.send_command('script', ['load', script], function (err, sha) {
        if (!err) {
            scriptList['hitGlobalBoss'] = sha;
        } else {
            console.log("loadScriptHitGlobalBoss error ", err);
        }
    }.bind(this));
}
...
exports.SetIncrbyLua = function (myHash, field, incrby, valueLimit, callback) {
    if (shaIncrbyLua == 0) {
        loadScriptIncrbyLua(function (ret) {
            shaIncrbyLua = ret;
            incrbyLuaExcute(myHash, field, incrby, valueLimit, callback);
        }.bind(this));
    } else {
        incrbyLuaExcute(myHash, field, incrby, valueLimit, callback);
    }
...
exports.CalcAwardMoney = function (BossAutoID, MoneyValue, TotalMoneyAward, callback) {
    try {
        var key = Config.Define.BossGlobal + BossAutoID;
        var name = Config.Define.BossGlobal_CurrentAwardMoney;
        this.SetIncrbyLua(key, name, MoneyValue, TotalMoneyAward, callback);
...
exports.CalcHitBoss = function (HPValue, TotalHP, callback) {
    try {
        var key = 'TestPer';
        var name = 'TestPerHash';
        var name2 = 'TestPerHashWinUserID';
        var uid = Math.floor(Math.random()*100000);
        execLuaScript('hitGlobalBoss',key, name, HPValue, TotalHP,name2,uid, callback);
    }
    catch (e) {
        callback(0);
